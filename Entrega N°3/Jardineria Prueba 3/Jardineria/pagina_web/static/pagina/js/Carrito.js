...
function ready() {
    ...
    for (var i = 0; i < botonesAgregarAlCarrito.length; i++) {
        ...
        button.addEventListener('click', agregarItemAlCarritoClicked);
    }
...
function agregarItemAlCarritoClicked(event) {
    var button = event.target;
    var item = button.parentElement;
    var titulo = item.getElementsByClassName('titulo-item')[0].innerText;
    var precioTexto = item.getElementsByClassName('precio-item')[0].innerText;
    var precio = parseFloat(precioTexto.replace('Precio: $', '').replace(',', '')); // Asegúrate de obtener el precio como número

    // Formateamos el precio con el signo $
    var precioFormateado = '$' + precio.toFixed(2); // Ajusta según tu necesidad de formato decimal

    var imagenSrc = item.getElementsByClassName('img-item')[0].src;

    agregarItemAlCarrito(titulo, precioFormateado, imagenSrc);
...
function actualizarTotalCarrito() {
    ...
    // Redondeamos el total al entero más cercano
...
function cargarCarrito() {
    ...
    if (carritoData) {
        for (var i = 0; i < carritoData.length; i++) {
            ...
            agregarItemAlCarrito(item.titulo, '$' + item.precio, item.imagenSrc); // Asegúrate de formatear el precio al cargar el carrito
        }
    }
}
